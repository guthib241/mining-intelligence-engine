...
def compress(words, h, rounds):
    ...
    w = np.zeros((words.shape[0], 64), dtype=np.uint64); w[:, :16] = words
    ...
    for i in range(rounds):
        S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25); ch = (e & f) ^ ((~e & M) & g)
        t1 = (hh + S1 + ch + K[i] + w[:, i]) & M
        S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22); maj = (a & b) ^ (a & c) ^ (b & c); t2 = (S0 + maj) & M
...
def make_messages(N, rng):
    ...
    w = np.zeros((N, 16), dtype=np.uint64); w[:, :13] = rng.integers(0, 2**32, size=(N, 13), dtype=np.uint64)
    w[:, 13] = 0x80000000; w[:, 15] = 52 * 8; return w
...
def avalanche(N, rounds, rng, flips=8):
    ...
    w = make_messages(N, rng); h = sha256_1block(w, rounds); out = []
    for _ in range(flips):
        wi, bi = rng.integers(0, 13), rng.integers(0, 32); w2 = w.copy(); w2[:, wi] ^= np.uint64(1) << np.uint64(bi)
        d = to_bits(sha256_1block(w2, rounds) ^ h); out.append(d.mean(0))
    P = np.array(out); return float(P.mean()), float(np.abs(P - 0.5).max()), float(2 / np.sqrt(N))
def learn(N, rounds, rng, bits=(0, 7, 31, 100, 255), test_frac=0.25):
    w = make_messages(N, rng); X = to_bits(w[:, :13]); Y = to_bits(sha256_1block(w, rounds)); n = int(N * (1 - test_frac))
    res = {}
    for b in bits:
        y = Y[:, b]; r = {}
        if len(np.unique(y[:n])) < 2:   # constant bit at low rounds: perfectly predictable
            maj = float((y[n:] == y[:n][0]).mean()); res[f"bit{b}"] = {"logreg": maj, "mlp": maj, "gbt": maj, "best": maj, "se": float(0.5 / np.sqrt(N - n)), "constant": True}; continue
        for name, mdl in [("logreg", LogisticRegression(max_iter=300)), ("mlp", MLPClassifier(hidden_layer_sizes=(256, 128), max_iter=60, early_stopping=True, random_state=0)), ("gbt", HistGradientBoostingClassifier(max_iter=150, max_depth=6))]:
            mdl.fit(X[:n], y[:n]); acc = float((mdl.predict(X[n:]) == y[n:]).mean()); r[name] = acc
        r["best"] = max(r.values()); r["se"] = float(0.5 / np.sqrt(N - n)); res[f"bit{b}"] = r
    best_over_bits = max(v["best"] for v in res.values()); return res, best_over_bits
def run(cfg):
    rng = np.random.default_rng(cfg.get("seed", 0)); N = cfg.get("N", 60000); ladder = cfg.get("rounds", [1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64])
    # sanity: full-round implementation must match hashlib
    import hashlib, struct
    w = make_messages(3, rng); ref = [hashlib.sha256(b"".join(struct.pack(">I", int(x)) for x in row[:13])).digest() for row in w]
    got = sha256_1block(w, 64); ok = all(bytes(b"".join(struct.pack(">I", int(x)) for x in got[i])) == ref[i] for i in range(3))
    if not ok:
        raise RuntimeError("SHA-256 implementation does not match hashlib")
    import json, time
    from ..infrastructure import PATHS
    cache = PATHS["reports"] / "cache" / f"sha_ladder_N{N}_seed{cfg.get('seed', 0)}.json"; cache.parent.mkdir(exist_ok=True)
    out = json.load(open(cache)) if cache.exists() else {"implementation_matches_hashlib": True, "N": N, "ladder": {}}
    budget = cfg.get("time_budget_s", 240); t0 = time.time()
    for r in ladder:
        if str(r) in out["ladder"]:
            continue
        if time.time() - t0 > budget:
            out["partial"] = True; json.dump(out, open(cache, "w"), indent=1); return {"status_note": "PARTIAL - rerun to continue", "done_rounds": list(out["ladder"]), "metrics": {}, "verdict": "INCOMPLETE", "next_action": "rerun same cfg"}
        rr = np.random.default_rng(cfg.get("seed", 0) + r)
        av_mean, av_maxdev, av_se = avalanche(20000, r, rr)
        res, best = learn(N, r, rr); se = 0.5 / np.sqrt(N * 0.25)
        z = (best - 0.5) / se
    ...
    full = out["ladder"]["64"]; nbits_tested = 5; bonf = stats.norm.ppf(1 - 0.05 / (nbits_tested * 3 * len(ladder)))
    out["full_round_verdict"] = "NO SIGNAL (chance)" if full["z_vs_chance"] < bonf else "SIGNAL - investigate"; out["bonferroni_z"] = round(float(bonf), 2)
    wall = [int(r) for r, v in out["ladder"].items() if v["z_vs_chance"] >= bonf]; out["rounds_with_learnable_structure"] = wall
